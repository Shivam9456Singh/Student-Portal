...
if (isset($_POST['searchBtn']) && !empty($_POST['searchQuery'])) {
    ...
    $whereClause .= " AND (name LIKE ? OR Guardians_name LIKE ? OR mobile LIKE ? OR email LIKE ?)";
                                            ...
                                            $sql = "SELECT * FROM student WHERE email_verified = 1";
                                            ...
                                            if ($result->num_rows > 0) {
                                                while($row = $result->fetch_assoc()) {
                                                   echo "<tr>";
													...
													echo "<td>";
if (!empty($row["transaction_image"])) {
echo "<a href='get_image.php?name=" . urlencode($row["name"]) . "' target='_blank' class='view-transaction transaction-link' style='color: black;'>View Reciept</a>";
} else {
    echo "Not Paid";
}
