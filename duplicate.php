...
$duplicate_error = ""; // Initialize the error message for duplicate entries
...
if (isset($_POST['signup'])) {
    ...
    $checkDuplicateQuery = "SELECT * FROM student WHERE name = '$name' OR email = '$email' OR mobile = '$mobile'";
    $result = mysqli_query($conn, $checkDuplicateQuery);

    if(mysqli_num_rows($result) > 0) {
        $duplicate_error = "Can't register with the same Name, Email, or Mobile.";
        $error = true;
    }
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
          ...
          <div class="form-row">
            ...
            <div class="form-group col-md-6">
              ...
              <input type="text" name="Guardians_name" class="form-control" value="" maxlength="50" required="" placeholder="Enter Father's/ Guardian's Name">
              <span class="text-danger"><?php if (isset($name_error)) echo $name_error; ?></span>
            </div>
          </div>
          ...
          <div class="form-row">
           ...
           <label>Address</label>
            <textarea name="address" class="form-control" rows="3" required="" placeholder=" Permanent address"></textarea>
